...
/* KPI CARDS (Centrage des nombres au milieu) */
.metric-card {
...
# =========================================================
# DATA HANDLING (Shared across pages)
# =========================================================
...
# =========================================================
    ...
    # MIXTE: Sélectionner ou Nouveau Client
    clients_existants = sorted(df_raw["Client"].unique().astype(str).tolist())
    options_clients = clients_existants + ["➕ Nouveau Client (Saisie manuelle)"]
    
    choix_client = st.selectbox("Sélectionner le Client", options_clients)

    if choix_client == "➕ Nouveau Client (Saisie manuelle)":
        client_final = st.text_input("Nom du Client Jdid")
    else:
    ...
    if st.session_state.devis_items:
        ...
        with col_final1:
            if st.button("📄 Générer le Devis PDF", use_container_width=True):
                st.success(f"Devis PDF prêt pour {client_final}")
        with col_final2:
